...
def pr2_mover(object_list):
    ...
    color_map = {'red':'left','green':'right'}
    objects = dict([ [item['name'],color_map[item['group']]] for item in object_list_param])
    group_pose = dict([ [group['name'],group['position']] for group in place_param])
    ...
    for i, object in enumerate(object_list):
        # 1- object_name
        if object.label not in objects:
            continue
    ...
    send_to_yaml('output_{}.yaml'.format(test_scene), yaml_list)
